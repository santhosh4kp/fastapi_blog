...
def update_blog_by_id(id: int, blog:UpdateBlog, db:Session, author_id:int):
    ...
    if not blog_in_db:
        return{"error":f"blog with {id} does not exists"}
    if not blog_in_db.owner_id== author_id:
...
def delete_by_id(id:int, db:Session,author_id:int):
    blog_in_db=db.query(Blog).filter(Blog.id==id)
    if not blog_in_db.first():
        return {"error": f"Cound not find the blog with id {id}"}
    if not blog_in_db.first().owner_id== author_id:
        return {"error": "Only the author can delete the blog"}
    
    blog_in_db.delete()
    db.commit()
    return {"msg": f"deleted the entry with id {id}"}
